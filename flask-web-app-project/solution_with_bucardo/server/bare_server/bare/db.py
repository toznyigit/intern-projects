...
def insert_user(IP, args):
    try:
        ...
        __log_act(args["username"],"REGISTER",IP)
        curr.close()
        conn.commit()
    except (Exception, psycopg2.DatabaseError) as error:
        print(error)
...
def check_user(username, email=None, enc_passwd=None):
    ...
    try:
        ...
        if enc_passwd:
            ...
            result = False
            curr.execute(f"SELECT * FROM user_table WHERE username = '{username}'")
            name = curr.fetchone()
            if name:
                result = True
        curr.close()
...
def update_user(username, IP, args):
    try:
        conn = get_db_connection()
        curr = conn.cursor()
...
def is_online(username):
    ...
    try:
        conn = get_db_connection()
        curr = conn.cursor()
        curr.execute(f"SELECT susername FROM session_table WHERE susername='{username}'")
        if curr.fetchone():
            result = True
        curr.close()
        conn.commit()
...
def __log_act(username, activity, IP):
    file_obj = open(log_file,'a')
    act = f'username: {username}\t| ipaddress: {IP}\t| logdatetime: {datetime.now()}\t| activity: {activity}\n'
    file_obj.write(act)
    file_obj.close()